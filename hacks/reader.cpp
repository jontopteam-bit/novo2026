...
// ✅ OFFSETS CONFIRMADOS
const uintptr_t BONE_OFFSET = 0x200;
const int BONE_STRIDE = 0x20;
...
void CGame::loop() {
	...
	while (true) {
		...
		// ✅ USA OFFSET 0x200 CONFIRMADO
		if (config::show_skeleton_esp || config::show_head_tracker) {
			if (player.gameSceneNode != 0) {
				player.boneArray = process->read<uintptr_t>(player.gameSceneNode + BONE_OFFSET);
...
void CPlayer::ReadHead() {
	...
	boneAddress = boneArray + 6 * BONE_STRIDE;
	bonePosition = g_game.process->read<Vector3>(boneAddress);
	bones.bonePositions["head"] = bonePosition;
}
...
void CPlayer::ReadBones() {
	...
	for (const auto& entry : boneMap) {
		...
		boneAddress = boneArray + boneIndex * BONE_STRIDE;
		bonePosition = g_game.process->read<Vector3>(boneAddress);
		bones.bonePositions[boneName] = bonePosition;
	}
}
